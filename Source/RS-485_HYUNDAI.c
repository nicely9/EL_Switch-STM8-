...
#define CLOSE	1
#define REQUEST	2
uint8_t Gu8_Gas_State = 0;	//0 : 해제, 1 : 차단
...
void RS_485_ID_RES(void)
{
    ...
    prime_num_1 = (uint8_t)((Gu16_Elec_1_Watt - (int)Gu16_Elec_1_Watt) * 10);	//전열1번 소수점을 *10 해서 정수로
	prime_num_2	= (uint8_t)((Gu16_Elec_2_Watt - (int)Gu16_Elec_2_Watt) * 10);	//전열2번 소수점을 *10 해서 정수로
	
    Elec_Watt_MSB = (uint8_t)((int)Gu16_Elec_1_Watt + (int)Gu16_Elec_2_Watt);
	Elec_Watt_LSB = (uint8_t)(prime_num_1 + prime_num_2);

	if(10 <= (prime_num_1 + prime_num_2))	//전열1, 2의 소수 값들의 합이 10을 넘으면
	{
		Elec_Watt_MSB += 1;					//정수 자리에 + 1
		Elec_Watt_LSB -= 10;				//소수 자리에 -10
	}

	pTx->buf[pTx->count++]	= 0xCC;
	pTx->buf[pTx->count++]	= pG_Config->Protocol_Type;		//프로토콜 타입
	pTx->buf[pTx->count++]	= Get_485_ID();					//ID 번호
	pTx->buf[pTx->count++]	= Get_485_ID();					//현대 프로토콜은 전등 ID과 전열 ID 동일함
...
/*#ifdef _ONE_SIZE_BATCH_BLOCK_MODEL_
    ...
    switch(pRx->count)
    {
        ...
        case 5:
            ...
            {
                if((pRx->buf[HD_F_STX] != NIS_LIGHT_ID_COMM_1) && (pRx->buf[HD_F_LEN] != NIS_LIGHT_ID_COMM_2) && (pRx->buf[HD_F_DEV] != HD_DEV_USS))
                {
                    pRx->count = 0;
                }
            }
...
void HYUNDAI_ACK(HYUNDAI_BUF *pRx)
{
    ...
    Light_Cnt	= (uint8_t)(pG_Config->LightCount + pG_Config->ThreeWayCount);
	...
	if(pRx->buf[HD_F_DEV] == HD_DEV_LIGHT)
	{
		...
		if(pRx->buf[HD_F_TYPE] == HD_TYPE_STATE_REQ)
		{
			if(Loc_LSB == HD_LOC_CIRCUIT_ALL)
			{
				...
				// 	}
				// }
				for(i = 1; i <= Light_Cnt; i++)
				{
					pTx->buf[pTx->count++] = HYUNDAI_LIGHT_Data_Conversion(HYUNDAI_LIGHT_ITEM_Sequence[i], HYUNDAI_ON_n_OFF);
				}
			}
			else
			{
...
void HYUNDAI_Data_Process(HYUNDAI_BUF	*pRx)
{
    ...
    Loc_MSB = (uint8_t)(pRx->buf[HD_F_LOC] >> 4);
    Loc_LSB = (uint8_t)(pRx->buf[HD_F_LOC] & 0x0F);
	...
	if(pRx->buf[HD_F_DEV] == HD_DEV_LIGHT)
	{
		if(Loc_LSB > pG_Config->LightCount + pG_Config->ThreeWayCount) return;
	}
...
void HYUNDAI_Data_Process(HYUNDAI_BUF	*pRx)
{
	...
	if(pRx->buf[HD_F_DEV] == HD_DEV_LIGHT_DIMMING)
	{
		if(Loc_LSB > pG_Config->DimmingCount)	return;
	}
	if(pRx->buf[HD_F_DEV] == HD_DEV_ELETRICITY || pRx->buf[HD_F_DEV] == HD_DEV_STANDBY_POWER)
	{
		if(Loc_LSB > pG_Config->ElectricityCount) return;
	}
				...
				if(pRx->buf[HD_F_TYPE] == HD_TYPE_STATE_REQ)
				{
					if(Loc_LSB == HD_LOC_CIRCUIT_ALL)
					{
						if(pRx->buf[HD_F_ARG_1] == HD_DEV_USS)	//일괄차단 상태요청
						{
							...
							{
								if((GET_Switch_State(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS)) == 0) && Gu8_Gas_State == CLOSE)	pTx->buf[pTx->count++] = HD_CMD_BLOCK;
							...
							else
							{
								...
								{
									if((GET_Switch_State(item2tsn(mapping_ITEM_BATCH_LIGHT_OFF)) == 0) && (GET_Switch_State(item2tsn(mapping_ITEM_GAS))))		pTx->buf[pTx->count++] = HD_CMD_BLOCK;
									else              																											pTx->buf[pTx->count++] = HD_CMD_UNBLOCK;     //일괄, 가스 둘중 하나라도 차단 해제면 UNBLOCK(0x02)	
								}
								else								//가스 차단 기능이 없을 때,
								{
									if(GET_Switch_State(item2tsn(mapping_ITEM_BATCH_LIGHT_OFF)) == 0)		pTx->buf[pTx->count++] = HD_CMD_BLOCK;
									else																	pTx->buf[pTx->count++] = HD_CMD_UNBLOCK;
								}
							}
						}
						else									//일괄 차단 전체 상태요청
						{
							...
							for(i = 0; i < Data_Cnt; i++)
							{
								if(pRx->buf[pTx->count] == HD_DEV_LIGHT)
								{
									...
									if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
									{
										if(GET_Switch_State(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS)))		pTx->buf[pTx->count++] = HD_CMD_UNBLOCK;
								...
								else if(pRx->buf[pTx->count] == HD_DEV_GAS)
								{
									if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
									{
										...
										if(Gu8_Gas_State == OPEN)												pTx->buf[pTx->count++] = HD_CMD_OPEN;		//가스 차단해제중이면 열림으로 응답
										else																	pTx->buf[pTx->count++] = HD_CMD_CLOSE;		//가스 차단 요청 or 차단이면 닫힘으로 응답				
									}
									else
									{
				...
				if(pRx->buf[HD_F_TYPE] == HD_TYPE_CONTROL_REQ)
				{
					if(Loc_LSB == HD_LOC_CIRCUIT_ALL)				//전체 회로 제어
					{
						if(pRx->buf[HD_F_ARG_1] == HD_DEV_USS)		//일괄 차단 제어
						{
							...
							if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
							{
								if(GET_Switch_State(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS)) == OFF && Gu8_Gas_State == CLOSE)
								{									//일괄, 가스 둘 다 차단 상태 일 때,
			...
			else if(pRx->buf[HD_F_SRV] == HD_SRV_OPEN_n_CLOSE)		//일괄가스 상태요청
			{
				if(pRx->buf[HD_F_TYPE] == HD_TYPE_STATE_REQ || pRx->buf[HD_F_TYPE] == HD_TYPE_CONTROL_REQ)
				{
					if(pRx->buf[HD_F_ARG_1] == HD_DEV_GAS)
					{
						...
						{
							if(Gu8_Gas_State == OPEN)							pTx->buf[pTx->count++] = HD_CMD_OPEN;		//가스 차단해제중이면 열림으로 응답
							else												pTx->buf[pTx->count++] = HD_CMD_CLOSE;		//가스 차단중이면 닫힘으로 응답
						}
						else
						{
							if(item2tsn(mapping_ITEM_GAS))
...
void HYUNDAI_Data_Process(HYUNDAI_BUF	*pRx)
{
	...
	if(Loc_MSB == (uint8_t)pG_Config->RS485_ID && (pRx->buf[HD_F_DEV] == HD_DEV_USS || pRx->buf[HD_F_DEV] == HD_DEV_ELEVATOR_CALL || pRx->buf[HD_F_DEV] == HD_DEV_EXTERNAL_CONTACT))
	{
		if(G_Debug == DEBUG_HOST)
		{
			printf("\nRX(HYUNDAI) : ");
						...
						for(i = 0; i < Data_Cnt; i++)
						{
							if(pRx->buf[HD_F_ARG_1 + cnt] == HD_DEV_GAS)
							{
								if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
								{
									...
									{
										if(Gu8_Gas_State != REQUEST)	//가스 요청중이 아닐 때는 열림으로 동작함.
										{
								...
								{
									if(item2tsn(mapping_ITEM_GAS))	//230628 추가
									{
										if(pRx->buf[HD_F_ARG_1 + cnt + 1] == HD_CMD_CLOSE)		BATCH_BLOCK_Control(SET__GAS_CLOSE_STATE);
										else if(pRx->buf[HD_F_ARG_1 + cnt + 1] == HD_CMD_OPEN)
										{
					...
					else if(pRx->buf[HD_F_SRV] == HD_SRV_OPEN_n_CLOSE)
					{
						if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
						{
							if(pRx->buf[HD_F_ARG_1] == HD_DEV_GAS)
							{
								...
								else if(pRx->buf[HD_F_ARG_2] == HD_CMD_OPEN)
								{
									if(Gu8_Gas_State != REQUEST)	//가스 요청중이 아닐 때는 열림으로 동작함.
									{
										BATCH_BLOCK_Control(SET__GAS_OPEN_STATE);
			...
			case HD_DEV_ELEVATOR_CALL:
				if(item2tsn(mapping_ITEM_ELEVATOR))
				{
					...
					else if(pRx->buf[HD_F_TYPE] == HD_TYPE_CONTROL_REQ)
					{
						if(pRx->buf[HD_F_SRV] == HD_SRV_ELEVATOR_UP_n_DOWN)
						{
							if((pRx->buf[HD_F_CMD] & HD_ARG_CALL_DOWN) == HD_ARG_CALL_DOWN)		//하향 호출이면
							{
								BATCH_BLOCK_Control(SET__ELEVATOR_CALL);						//엘리베이터 호출
								HYUNDAI_ACK(pRx);
							}
							else if((pRx->buf[HD_F_CMD] & HD_ARG_CALL_UP) == HD_ARG_CALL_UP)	//230220 check
							{
								HYUNDAI_ACK(pRx);
							}	//wsm 240530
...
void BATCH_BLOCK_STATE_Process(void)
{
	...
	{
		if(Gu16_GAS_Off_Tmr == 0)	//타이머 변수는 동일하게 사용함
		{
			if(Gu8_Gas_State == REQUEST)		//타이머가 0이 되면 LED OFF
			{
				BATCH_BLOCK_Control(SET__GAS_OPEN_STATE);
			}
		}
...
void BATCH_BLOCK_Control(uint8_t control)
{
	...
	switch(control)
	{
		case SET__BATCHLIGHT_OFF:
			...
			if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
			{
				if(GET_Switch_State(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS)))
				{
					EventCtrl(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS), OFF);
				}
			}
			break;
		case SET__BATCHLIGHT_ON:
			...
			if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
			{
				if(GET_Switch_State(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS)) == 0)
				{
					EventCtrl(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS), ON);
				}
			}
			break;
		...
		case SET__GAS_CLOSE_REQUEST:
			if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
			{
				if(Gu8_Gas_State != CLOSE)
				{
					Gu16_GAS_Off_Tmr	= 60;
					Gu8_Gas_State = REQUEST;
					Beep(ON);
					printf("CMD : REQUEST, State : !CLOSE -> REQUEST\r\n");
				}
				else
				{
					printf("CMD : REQUEST, State : CLOSE\r\n");
				}
			}
			else
			{
		...
		case SET__GAS_CLOSE_STATE:
			if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
			{
				if(Gu8_Gas_State != CLOSE)	//가스 차단 상태 아닐 때(열림, 요청)
				{
					Gu8_Gas_State = CLOSE;
					Beep(ON);
					printf("CMD : CLOSE, State : !CLOSE -> CLOSE\r\n");
				}
			}
			else
			{
		...
		case SET__GAS_OPEN_STATE:
			if(item2tsn(mapping_ITEM_BATCH_LIGHT_n_GAS))
			{
				if(Gu8_Gas_State != OPEN)	//가스 열림 상태 아닐 때(차단, 요청)
				{
					Gu8_Gas_State = OPEN;
					Beep(ON);
					printf("CMD : OPEN, State : !OPEN -> OPEN\r\n");
				}
			}
			else
			{
